...
public class Thought_Situational_WearingDesiredApparel_WLF : Thought_Situational
{
    ...
    protected override ThoughtState CurrentStateInternal()
    {
        ...
        foreach (Precept item in allPreceptsAllowingSituationalThought)
        {
            if (item is Precept_Apparel precept_Apparel && (precept_Apparel.TargetGender == Gender.None || precept_Apparel.TargetGender == pawn.gender))
            {
                ...
                if (pawn.gender != Gender.Female || !CoversHead(precept_Apparel.apparelDef))
                {
                    num++;
                }
            }
    ...
    private bool CoversHead(ThingDef thingDef)
    {
        foreach (BodyPartGroupDef bodyPartGroupDef in thingDef.apparel.bodyPartGroups)
        {
